...
import { getPages, getWorkspaces } from '@/lib/store';
...
export default function PublicWorkspaceIndex({ params }: Props) {
  ...
  const publicPages = getPages().filter(
    (p) =>
      p.visibility === 'public' &&
      p.status === 'published' &&
  ...
  // Sort newest first
  publicPages.sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
    ...
    <div className="min-h-screen bg-white text-gray-900 flex flex-col">
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-6 py-12">
        ...
        <header className="mb-10">
          <p className="text-sm text-indigo-600 font-medium mb-2 uppercase tracking-wide">
            Public workspace
          </p>
            ...
            {publicPages.map((page) => (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                        ...
                        <>
                          ...
                          <div className="flex items-center gap-1.5 flex-wrap">
                            {page.tags.map((tag) => (
                              <span
                                key={tag}
                                className="text-xs text-indigo-600 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-full"
                              >
                                {tag}
                              </span>
                            ))}
                  ...
                  <span
                    className="text-gray-300 group-hover:text-indigo-400 transition-colors text-lg mt-0.5 flex-shrink-0"
                    aria-hidden="true"
                  >
                    &#8594;
                  </span>
      ...
      <footer className="border-t border-gray-100 py-8 px-6 mt-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between text-sm text-gray-400">
          <Link href="/" className="font-semibold text-gray-600 hover:text-gray-900 transition-colors">
            Talion
          </Link>
